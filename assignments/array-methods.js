...
runners.forEach(function(name) {
  let combinedName = `${name.first_name} ${name.last_name}`;
  fullName.push(combinedName);
});
...
runners.map(
  (name = name => {
    let upper = name.first_name.toUpperCase();
    allCaps.push(upper);
  })
);
...
const largePeople = runners.filter(shirt => shirt.shirt_size === 'L');
...
const cheapSkates = runners.filter(lowball => lowball.donation < 10);
lowDonors.push(cheapSkates);
...
const getDonations = runners.reduce(function(acc, amount) {
  acc += amount.donation;
  averageDonation = acc / runners.length;
  return acc;
}, 0);
...
const ballers = runners.filter(baller => baller.donation > 200);
highDonors.push(ballers);
